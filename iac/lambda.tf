...
data "archive_file" "lambda_code_consumer" {
  type        = "zip"
  source_dir  = ".././src/consumer/"
  output_path = "./consumer/export/src/lambda_function.zip"
}


resource "aws_lambda_function" "aws_lambda_function_consumer" {

  filename         = data.archive_file.lambda_code_consumer.output_path
  function_name    = "lambda_${var.project}_consumer_kafka"
  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.lambda_code_consumer.output_base64sha256
  ...
  environment {
    variables = {
      NAM_KAFKA_BROKER = aws_instance.example.public_dns,
      NAM_TOPIC        = "crypto_topic"
      NAM_BUCKET        = aws_s3_bucket.bucket_s3.id
    }
  }
